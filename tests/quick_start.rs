...
fn quick_start_comprehensive_demo() -> anyhow::Result<()> {
    ...
    println!(
        "✅ Created posts: '{}' by Alice, '{}' by Bob",
        saved_alice_post.title, saved_bob_post.title
    );
    ...
    let users_subscriber =
        alice_db.subscribe_to_query::<User>("SELECT * FROM User ORDER BY name", &[])?;
    ...
    println!(
        "   Alice's database has {} tracked changes",
        alice_changes.len()
    );
    println!(
        "   Bob's database has {} tracked changes",
        bob_changes.len()
    );
    ...
    assert_eq!(
        all_users_in_alice.len(),
        2,
        "Alice should have 2 users after sync"
    );
    assert_eq!(
        all_users_in_bob.len(),
        2,
        "Bob should have 2 users after sync"
    );
    assert_eq!(
        all_posts_in_alice.len(),
        2,
        "Alice should have 2 posts after sync"
    );
    assert_eq!(
        all_posts_in_bob.len(),
        2,
        "Bob should have 2 posts after sync"
    );

    // Verify both posts are now published
    let published_posts_alice: Vec<Post> =
        alice_db.query("SELECT * FROM Post WHERE published = ?", &[&true])?;
    let published_posts_bob: Vec<Post> =
        bob_db.query("SELECT * FROM Post WHERE published = ?", &[&true])?;

    assert_eq!(
        published_posts_alice.len(),
        2,
        "Both posts should be published in Alice's DB"
    );
    assert_eq!(
        published_posts_bob.len(),
        2,
        "Both posts should be published in Bob's DB"
    );
...
fn quick_start_reactive_queries_demo() -> anyhow::Result<()> {
    ...
    println!(
        "✅ Added users: {} and {}",
        saved_user1.name, saved_user2.name
    );
